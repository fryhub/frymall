...
<?php if(!empty($data)): ?><div class="tabs newmain background">
	...
	<nav class="clearfix tabs-main">
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><dl class="list3" <?php if(($i) == "1"): ?>style="display:block"<?php endif; ?> >
			...
			<?php else: ?>
				<?php if(is_array($li["data"])): $i = 0; $__LIST__ = $li["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd class="akmall-item-list"> 
					<a href="<?php echo U('Index/order',array('id'=>$vo['sn']));?>" title="<?php echo ($vo["name"]); ?>" class="info"> 
					...
					<em><strong><?php echo lang('symbol'); echo ($vo["price"]); ?></strong></em>
					</div>
