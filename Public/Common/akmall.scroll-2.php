<?php
$html = "<style>.akmall-scroll2{height:160px;overflow:hidden;line-height:30px;border-radius: 5px; box-shadow: 0 1px 2px 2px #E4E4E4;background: #fafafa;margin:10px 5px}.akmall-scroll2 li span{margin-right:10px;}</style>";
...
if($akmallConfig['real_notice']==1){
	...
	for($i=0;$i<50;$i++){
		...
		$html .= "<li ".($i%2 == 0?"class='even'":'')."><span class='akmall-badge'>{$pp}</span><span class='akmall-mobile'>{$nn}*[{$mm}]</span><span class='akmall-item'>{$pro}</span><span class='akmall-date'>{$time[$rand]}</span></li>";
	}	
}
$html .= "</ul></div></div>";
