...
class Page {
	...
	protected $config  =    array('header'=>'Items','prev'=>'<','next'=>'>','first'=>'<<','last'=>'>>','theme'=>' %totalRow% %header% %nowPage%/%totalPage% Page %first% %upPage%  %prePage%  %linkPage%  %nextPage% %downPage% %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        if($this->url){
            ...
            $url            =  C('URL_MODEL')=='2' && !isset($_GET['p'])?U('/',$parameter):U('',$parameter);
            //$url            =  U('',$parameter);
